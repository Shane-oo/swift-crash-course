...
if let yourCar = try? Car(manufacturer: "Tesla"){
    "Successs, you car = \(yourCar)"
} else {
    "failed to create car"
}


// not recommended never use "try!"
...
} catch NameErrors.fistNameIsInvalid {
...
{
    guard value > 0 else {
        return Result.failure(IntegerErrors.noPositiveIntegerBefore(thisValue: value))
    }
...
func performGet(forValue value: Int){
    ...
    case let .failure(error):
        ...
        case let .noPositiveIntegerBefore(thisValue):
            "no postivie integer before \(thisValue)"
            break
        }
        break
    }
